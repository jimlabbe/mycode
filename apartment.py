...
#search the entire html page looking for apartment listings 
def make_soup(page_html): 
    ...
    with open("/home/student/static/housing.csv", 'w', newline='') as f:
        ghostwriter = writer(f)                                  #obj to print each line
        ...
        for list in lists:
            ...
            ghostwriter.writerow(info) #prints line by line, then loops for the next listing

def main():
    page_html = get_url()
    make_soup(page_html)
    print("Soup complete! Bon appetit!")
